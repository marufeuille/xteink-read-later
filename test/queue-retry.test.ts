import { describe, expect, it } from 'vitest'
import {
  CLIP_QUEUE_MAX_RETRIES,
  parseClipQueueMessage,
  shouldRetryClipAttempt,
  shouldRetryClipError,
} from '../src/queue/clip'
import { asClipJobId, asClipRunId, parseHttpUrl } from '../src/types'

function url() {
  const parsed = parseHttpUrl('https://example.com/a')
  if (parsed === null) {
    throw new Error('url')
  }
  return parsed
}

describe('clip queue retry', () => {
  it('retries translate_failed and fetch_failed until the last attempt', () => {
    expect(shouldRetryClipError({ kind: 'translate_failed' }, 1)).toBe(true)
    expect(shouldRetryClipError({ kind: 'fetch_failed' }, CLIP_QUEUE_MAX_RETRIES)).toBe(true)
    expect(shouldRetryClipError({ kind: 'translate_failed' }, CLIP_QUEUE_MAX_RETRIES + 1)).toBe(false)
  })

  it('does not retry a fetch failure that a site recovery marked terminal', () => {
    expect(
      shouldRetryClipError(
        { kind: 'fetch_failed', terminal: true },
        1,
      ),
    ).toBe(false)
  })

  it('retries epub_failed once, then fails', () => {
    expect(shouldRetryClipError({ kind: 'epub_failed' }, 1)).toBe(true)
    expect(shouldRetryClipError({ kind: 'epub_failed' }, 2)).toBe(false)
  })

  it('does not retry extract_failed or payload_too_large', () => {
    expect(shouldRetryClipError({ kind: 'extract_failed' }, 1)).toBe(false)
    expect(shouldRetryClipError({ kind: 'payload_too_large' }, 1)).toBe(false)
    expect(shouldRetryClipError({ kind: 'invalid_url' }, 1)).toBe(false)
  })

  it('retries unexpected attempts until the last delivery', () => {
    expect(shouldRetryClipAttempt(1)).toBe(true)
    expect(shouldRetryClipAttempt(CLIP_QUEUE_MAX_RETRIES)).toBe(true)
    expect(shouldRetryClipAttempt(CLIP_QUEUE_MAX_RETRIES + 1)).toBe(false)
  })

  it('accepts {jobId,runId,url} only and ignores extra keys', () => {
    const jobId = asClipJobId('job_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
    const runId = asClipRunId('run_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
    const parsed = parseClipQueueMessage({
      jobId,
      runId,
      url: url(),
      extra: 'must-not-be-required',
    })
    expect(parsed).toEqual({ jobId, runId, url: url() })
    expect(parseClipQueueMessage({ jobId, url: url() })).toBeNull()
    expect(parseClipQueueMessage({ jobId, runId, url: 'ftp://example.com/x' })).toBeNull()
    expect(parseClipQueueMessage({ url: url() })).toBeNull()
  })
})
