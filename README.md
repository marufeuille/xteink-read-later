...
号全体の目安は約 5 分。電子ペーパー向けに 1 分 400 字として、選んだ本数で各要約の字数を割る。1 本あたり 400 字を超えない。5 本なら 400 字、10 本なら 200 字。1 本だけの号は 400 字で止め、5 分まで引き伸ばさない。本文は抽出した全文からだけ作り、タイトルやフィード抜粋からは作らない。要約が 0 件の日、または実行が失敗した日は、デイリーをカタログから外す。クリップと購入 EPUB は残す。識別子と旧号は [docs/daily-opds.md](docs/daily-opds.md)。OPDS のデイリーは最新 1 冊だけ。
