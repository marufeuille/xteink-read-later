#!/usr/bin/env bash
# Create wrangler.jsonc CLIP_QUEUE if it is missing. Do not print tokens.
set -euo pipefail
...
echo '::error::Failed to create queue xteink-read-later-clip. Add Account permission Workers Queues: Edit to CLOUDFLARE_API_TOKEN, or create the queue once with: npx wrangler queues create xteink-read-later-clip'
exit 1
