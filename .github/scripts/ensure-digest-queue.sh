#!/usr/bin/env bash
# Create wrangler.jsonc DIGEST_QUEUE if it is missing. Do not print tokens.
set -euo pipefail
...
echo '::error::Failed to create queue xteink-read-later-digest. Add Account permission Workers Queues: Edit to CLOUDFLARE_API_TOKEN, or create the queue once with: npx wrangler queues create xteink-read-later-digest'
exit 1
